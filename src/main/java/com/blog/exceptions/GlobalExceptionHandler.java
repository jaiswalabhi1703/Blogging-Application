...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<ApiResponse> tokenRefreshExceptionHandler(TokenRefreshException ex) {
		return new ResponseEntity<>(new ApiResponse(ex.getMessage(), false), HttpStatus.UNAUTHORIZED);
	}

	// 403 - authenticated but lacking the required role (e.g. @PreAuthorize)
	@ExceptionHandler(AccessDeniedException.class)
	public ResponseEntity<ApiResponse> accessDeniedExceptionHandler(AccessDeniedException ex) {
		return new ResponseEntity<>(new ApiResponse("You do not have permission to perform this action", false),
